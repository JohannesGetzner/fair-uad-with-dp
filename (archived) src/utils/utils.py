...
def save_checkpoint(path: str, model: nn.Module, optimizer, step: int, config: Dict):
    ...
    checkpoint = {'model': model.state_dict(), 'config': config, 'step': step, "optimizer": optimizer.state_dict()}
    torch.save(checkpoint, path)


def save_checkpoint_dp(path: str, model: nn.Module, optimizer, privacy_engine, step: int, config: Dict):
    if not os.path.exists(os.path.dirname(path)):
        os.makedirs(os.path.dirname(path))
    checkpoint = {'model': model.state_dict(), 'config': config, 'step': step,
                  "optimizer": optimizer.original_optimizer.state_dict(), "accountant": privacy_engine.accountant}
    torch.save(checkpoint, path)
...
def init_wandb(config, log_dir: str, group_name: str, job_type: str, sweep=False):
    ...
    if sweep:
        ...
        run = wandb.init(
            project=config.wandb_project, config=dict(config), group=group_name, dir=log_dir, tags=wandb_tags,
